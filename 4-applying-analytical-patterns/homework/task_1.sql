...
SELECT
	...
	CASE 
        ...
        WHEN ts.season IS NULL AND ls.last_active_season = ts.season - 1 THEN 'Retired'
        WHEN COALESCE(ts.season, ls.last_active_season) + 1 >= ls.season THEN 'Continued Playing'
        ELSE 'Stayed Retired'
	END AS yearly_active_state,
	COALESCE(ls.seasons_active,
		ARRAY[]::INTEGER[])
		|| CASE WHEN
			ts.player_name  IS NOT NULL
